...
import edu.ufl.cise.gatorraider.models.*;
 ...
 * and DefenderController respectively. The game may be executed using JreExec.
 */
...
//	protected BitSet pills,powerPills;
	...
	//Constructor
	protected _Game(String gamePath)
	{
		gameDir = new DataFile(gamePath);
	}

	private _Game(DataFile _gameDir)
	{
		gameDir = _gameDir;
	}
	...
	protected void init()
	{
		for (int i = 0; i < mazes.length; i++)
			if (mazes[i] == null)
				mazes[i] = new _Maze(gameDir, pathMazes + System.getProperty("file.separator") + nodeNames[i], pathMazes + System.getProperty("file.separator") + distNames[i]);
	}
	
	//Creates an exact copy of the game
	public Game copy()
	{
		_Game copy=new _Game(gameDir);
